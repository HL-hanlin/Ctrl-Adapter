...
class AdapterSpatioTemporal(nn.Module):
    def __init__(
        ...
        self.up = up # upsample the input features by factor of 2, this is set as True for diffusion models based on SDXL backbone (e.g., SDXL, Hotshot-XL)
            ...
            if self.add_spatial_resnet:
                ...
                spatial_resnets.append(
                    ResnetBlock2D(
                        ...
                        up = self.up if i==0 else False, # only the 1st layer in SDXL-based backbone needs upscale 
                    )
                )
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            ...
            ### 1.1 spatial resnet ###
            if self.add_spatial_resnet:
                hidden_states = self.spatial_resnets[i](hidden_states, resnet_temb) # bf c h w -> bf c h w
            ...
            if not self.add_spatial_resnet and not self.add_temporal_resnet and i==0 and self.up: # for sdxl only 
