"""
This file is mostly copied from diffusers library: https://github.com/huggingface/diffusers/blob/v0.27.2/src/diffusers/pipelines/i2vgen_xl/pipeline_i2vgen_xl.py
...
from utils.ctrl_adapter import ControlNetAdapter, ControlNetHelper, ControlNetRouter
...
class I2VGenXLControlNetAdapterPipeline(DiffusionPipeline):
    ...
    def _get_add_time_ids(self, original_size, crops_coords_top_left, target_size, dtype):
        ...
        # TODO: addition_time_embed_dim
        passed_add_embed_dim = (0 * len(add_time_ids) + self.helper.text_encoder.config.projection_dim)
                ...
                if self.router is not None:
                    ...
                    if mid_block_weights is not None:
                        ...
                        for e in range(num_experts):
                            mid_block_res_sample_merged = mid_block_res_sample_merged + mid_block_res_sample[e] * mid_block_weights.repeat_interleave(num_frames, dim = 0)[e]
                    else:
                    ...
                    for k in range(num_routers):
                        for e in range(num_experts):
                            down_block_res_samples_merged[k] = down_block_res_samples_merged[k] + down_block_res_samples[e][k] * down_block_weights[k].repeat_interleave(num_frames, dim = 0)[e] 
    ...
    def encode_controlnet_prompt(
        ...
        if prompt_embeds is None:
            ...
            #with torch.no_grad():
            if clip_skip is None:
                prompt_embeds = self.helper.text_encoder(text_input_ids.to(device), attention_mask=attention_mask)
                pooled_prompt_embeds = prompt_embeds[1]
                prompt_embeds = prompt_embeds[0]
            else:
                #with torch.no_grad():
                prompt_embeds = self.helper.text_encoder(text_input_ids.to(device), attention_mask=attention_mask, output_hidden_states=True)
                #pooled_prompt_embeds = self.helper.text_encoder(text_input_ids.to(device))
                pooled_prompt_embeds = prompt_embeds[1]
                # Access the `hidden_states` first, that contains a tuple of
                # all the hidden states from the encoder layers. Then index into
                # the tuple to access the hidden states from the desired layer.
                ...
                # obtaining the final prompt representations passes through the LayerNorm
                # layer.
                prompt_embeds = self.helper.text_encoder.text_model.final_layer_norm(prompt_embeds)

        if self.helper.text_encoder is not None:
            prompt_embeds_dtype = self.helper.text_encoder.dtype
        elif self.unet is not None:
            prompt_embeds_dtype = self.unet.dtype
        else:
            prompt_embeds_dtype = prompt_embeds.dtype
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if hasattr(self.helper.text_encoder.config, "use_attention_mask") and self.helper.text_encoder.config.use_attention_mask:
                attention_mask = uncond_input.attention_mask.to(device)
            else:
                attention_mask = None
            #with torch.no_grad():
            negative_prompt_embeds = self.helper.text_encoder(
                uncond_input.input_ids.to(device),
                attention_mask=attention_mask,
            )
