...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalControlNetMixin):
    ...
    def forward(
        ...
        """
        ###
        The following lines are added to the original controlnet.py file to support: 
        (1) latents skipping (mentioned in sec 3.2 of our paper https://arxiv.org/pdf/2404.09967.pdf)
        (2)timestep embedding skipping (experimental)
        """
        sample_conv_in = self.conv_in(sample)
        if skip_conv_in:
            #print("skip conv in")
            sample = torch.zeros_like(sample_conv_in).to(sample_conv_in.device)
        else:
            sample = sample_conv_in
